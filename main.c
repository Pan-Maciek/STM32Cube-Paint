...
void draw(uint32_t color, int x, int y, int r) {
...
static void netconn_thread(void const *arg) {
  xprintf("netconn_thread\n");
  xprintf("waiting for ip\n");
  while (ip4_addr1_16(netif_ip4_addr(&gnetif)) == 0) {
    xprintf(".");
    vTaskDelay(100);
  }
  displayOwnIp();
  ...
  ip_addr_t remote_ip;
  remote_ip.addr = IP4_ADDR(&remote_ip, 192, 168, 1, 197);

  conn = netconn_new(NETCONN_TCP);
  netconn_connect(conn, &remote_ip, 3000);
  connected = 1;

  // varibles for reciving data
  struct netbuf *inbuf;
      ...
      if (netconn_err(conn) == ERR_OK) {
        ...
        if (buflen > 0) {
          if (buf[0] == 'c')
            ...
            sscanf(buf + 1, "%i %i %i %i %i %i", &r, &g, &b, &R, &x, &y);
            draw(rgb_color(r, g, b), x, y, R);
          }
        }
        netbuf_delete(inbuf);
      }
...
int h = 230, s = 78, l = 77, R = 10;
int r = 151, g = 166, b = 242;
uint32_t hslToRgb(int h0, int s0, int l0) {
  float h = h0 / 360.0, s = s0 / 100.0, l = l0 / 100.0;
  float R, G, B;
  ...
  return 0xFF000000 | ((r = to255(R)) << 16) | ((g = to255(G)) << 8) | (b = to255(B));
}
...
void draw_ui() {
  ...
  for (int i = 0; i < 2; i++) {
    BSP_LCD_FillRect(5, i * 25 + 5, 20, 20);
  }

  BSP_LCD_SetTextColor(hslToRgb(h, 100, 50));
  BSP_LCD_FillRect(5, 5 + (0 + 3) * 25, 20, 20);

  BSP_LCD_SetTextColor(hslToRgb(h, s, 50));
  BSP_LCD_FillRect(5, 5 + (1 + 3) * 25, 20, 20);

  BSP_LCD_SetTextColor(hslToRgb(0, 0, l));
  BSP_LCD_FillRect(5, 5 + (2 + 3) * 25, 20, 20);

  BSP_LCD_SetTextColor(hslToRgb(h, s, l));
  BSP_LCD_FillRect(5, 2 * 25 + 5, 20, 20);

  BSP_LCD_SelectLayer(0);
}

int in_range(int min, int max, int val) { return min <= val && val <= max; }
...
void onTouch(int x, int y) {
  if (in_range(5, 30, x)) {
    ...
    if (i == 0)
      ...
      R = max(3, R - 1);
    else if (i == 2 && last_i != 2) {
      netconn_write(conn, "c\n", 2, NETCONN_NOCOPY);
    } else {
      if (i == 3) {
        h = (h + 1) % 360;
      } else if (i == 4) {
    ...
    sprintf(message, "d %i %i %i %i %i %i\n", r, g, b, R, x, y);
    int size = strlen(message);
    netconn_write(conn, message, size, NETCONN_NOCOPY);
  }
}
...
void StartDefaultTask(void const *argument) {
  ...
  MX_LWIP_Init(); // initialize lwip

  osThreadDef(netconn_thread, netconn_thread, osPriorityNormal, 0, 1024); // define network thread
  netconn_thread_handle = osThreadCreate(osThread(netconn_thread), NULL); // start network thread
  ...
  while (connected == 0) { // wait for connection
    vTaskDelay(100);
  }
  draw_ui();
  ...
  while (1) {
    ...
    for (int i = 0; i < TS_State.touchDetected; i++) {
      onTouch(TS_State.touchX[i], TS_State.touchY[i]);
    }
    vTaskDelay(10);
  }
}
